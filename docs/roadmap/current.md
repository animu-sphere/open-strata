...
now carry schema-1 OpenUSD compatibility cells. A cell resolves compiler,
C++ standard, native runtime boundary, Python, and oneTBB versions from the CY
`core` map together with their named providers. The approved Linux x86_64 cells
...
exists, accepts `--openusd-variant`, refuses unsupported builder/variant pairs
and conflicting compatibility overrides before launch, and records the exact
resolved selection in schema-5 runtime identity and the exported producer
manifest. Targets without an approved cell retain the legacy unclassified build
