...
`passed`. `runtime validate` then probes the normalized cell's native OpenGL and
Vulkan loaders without requiring SDK utilities and updates only `loader`;
...
success. Normalized graphics runtimes must persist a passing loader observation
before export.
