...
`<Name>Config.cmake` or lowercase `<name>-config.cmake` in a standard CMake
config-mode search location in the verified SDK inventory. This structural
check is deterministic and does not execute target CMake package code; use
`runtime validate --sdk --cmake-package <Name>` before export for the opt-in
configure probe. A legacy composition without an SDK cannot become a consumer
package.
