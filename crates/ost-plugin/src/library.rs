...
impl Library {
    pub fn load(root: &Utf8Path) -> Result<Self> {
        ...
        if !is_portable_id(&manifest.library.id) {
            return Err(Error::config(format!(
                "library.id '{}' is not a portable identifier",
                manifest.library.id
            )));
        }
        satisfies(&manifest.library.version, &manifest.library.version).map_err(|error| {
            Error::config(format!(
                "library '{}' has an invalid version '{}': {error}",
                manifest.library.id, manifest.library.version
            ))
        })?;
        if manifest.cmake.package.trim().is_empty() {
            return Err(Error::config("library cmake.package must not be empty"));
        }
        if manifest.cmake.target.trim().is_empty() || !manifest.cmake.target.contains("::") {
            return Err(Error::config(format!(
                "library cmake.target '{}' must be a non-empty exported target such as Package::Target",
                manifest.cmake.target
            )));
        }
        for directory in &manifest.runtime.directories {
