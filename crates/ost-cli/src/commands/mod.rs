...
/// and `usdview` enable USD imaging/host tooling and are just as dependent, so
/// a name-prefix test alone reports a lookdev profile as needing nothing. This lives here because two
...
pub(crate) fn needs_openusd(capability: &str) -> bool {
    capability.starts_with("usd-") || matches!(capability, "hydra-preview" | "usdview")
}
