...
_ACTIVATED = False
_FINDER = None
_ACTIVE_RUNTIME = "_OST_CONSUMER_ACTIVE_RUNTIME"
...
def _binding_matches(path: Path, expected: dict) -> bool:
    try:
...
def _write_binding(path: Path, binding: dict) -> None:
...
def _reexec(environment: dict[str, str]) -> None:
    original = getattr(sys, "orig_argv", None)
    arguments = [sys.executable, *(original[1:] if original else sys.argv)]
    if sys.platform == "win32":
...
def activate() -> Path:
    ...
    if _ACTIVATED:
        ...
        return _cache_root() / manifest["runtime"]["runtime_digest"].removeprefix("sha256:")
    ...
    binding_path = prefix.with_name(
        prefix.name + "." + digest.removeprefix("sha256:") + ".consumer.json"
    )
    verify_artifact = not prefix.exists() or not _binding_matches(binding_path, binding)
    ...
    if not prefix.exists():
        ...
        try:
            ...
            shutil.rmtree(temporary, ignore_errors=True)
            raise
    observed = _ost("runtime", "env", "--composition", str(prefix))
    if observed.get("runtime_digest") != runtime_digest or observed.get("target") != binding["target"]:
        raise RuntimeError("cached OpenStrata runtime identity does not match the consumer package")
    if verify_artifact:
        _write_binding(binding_path, binding)

    environment = dict(observed["env"])
    if os.environ.get(_ACTIVE_RUNTIME) != runtime_digest:
        inherited = os.environ.copy()
        inherited.update(environment)
        inherited[_ACTIVE_RUNTIME] = runtime_digest
        _reexec(inherited)
...
class _PublicApiFinder(importlib.abc.MetaPathFinder):
    def __init__(self, entrypoints: tuple[str, ...]) -> None:
        self.entrypoints = entrypoints

    def find_spec(self, fullname: str, path=None, target=None):
        if not any(name == fullname or name.startswith(fullname + ".") for name in self.entrypoints):
            return None
        try:
...
def install() -> None:
    ...
    if _FINDER is not None:
        return
    manifest = json.loads(_MANIFEST.read_text(encoding="utf-8"))
    _FINDER = _PublicApiFinder(tuple(manifest["public_api"]["entrypoints"]))
...
function bindingFor(manifest) {
  return {
    artifact_digest: manifest.runtime.artifact_digest,
    runtime_digest: manifest.runtime.runtime_digest,
    target: manifest.runtime.target,
  };
}

function bindingMatches(bindingPath, expected) {
  try { return JSON.stringify(JSON.parse(fs.readFileSync(bindingPath, 'utf8'))) === JSON.stringify(expected); }
  catch (_) { return false; }
}

function writeBinding(bindingPath, binding) {
  const temporary = `${bindingPath}.${crypto.randomUUID()}.tmp`;
  try {
    fs.writeFileSync(temporary, `${JSON.stringify(binding)}\n`, 'utf8');
    try { fs.renameSync(temporary, bindingPath); }
    catch (error) {
      if (!bindingMatches(bindingPath, binding)) throw error;
    }
  } finally {
...
function activate() {
  if (activatedPrefix) return activatedPrefix;
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  const binding = bindingFor(manifest);
  const digest = binding.artifact_digest;
  const prefix = path.join(cacheRoot(), binding.runtime_digest.replace(/^sha256:/, ''));
  const bindingPath = `${prefix}.${digest.replace(/^sha256:/, '')}.consumer.json`;
  const verifyArtifact = !fs.existsSync(prefix) || !bindingMatches(bindingPath, binding);
  if (verifyArtifact) {
  ...
  const observed = ost('runtime', 'env', '--composition', prefix);
  if (observed.runtime_digest !== binding.runtime_digest || observed.target !== binding.target) {
    throw new Error('cached OpenStrata runtime identity does not match the consumer package');
  }
  if (verifyArtifact) writeBinding(bindingPath, binding);
  const environment = observed.env;
  for (const [key, value] of environment) process.env[key] = value;
  Module._initPaths();
  activatedPrefix = prefix;
  return activatedPrefix;
}
    ...
    let (filename, private_root) = match manifest.package.kind {
        ConsumerPackageKind::PythonWheel => {
            ...
            write_generated(
                ...
                &format!("import {private}; {private}.install()\n"),
            )?;
...
fn copy_adapter(adapter: &Utf8Path, destination: &Utf8Path, output: &Utf8Path) -> Result<()> {
    let root = fs::canonicalize(adapter).map_err(|error| Error::io(adapter.to_string(), error))?;
    let root = Utf8PathBuf::from_path_buf(root)
        .map_err(|path| Error::usage(format!("adapter path must be UTF-8: {}", path.display())))?;
    let output = fs::canonicalize(output).map_err(|error| Error::io(output.to_string(), error))?;
    if output == root.as_std_path() {
        return Err(Error::usage(
            "consumer package output directory must not be the adapter directory",
        ));
    }
    let excluded = output.starts_with(root.as_std_path()).then_some(output);
    fn visit(
        root: &Utf8Path,
        current: &Utf8Path,
    ...
    visit(&root, &root, destination, excluded.as_deref())
}
...
fn derived_wheel_tag(target: &str) -> Result<String> {
    ...
    let platform = if lower.contains("windows-x86_64") {
        ...
        let floor = lower
            .split('-')
            .find_map(|part| part.strip_prefix("glibc"))
            .filter(|digits| digits.len() >= 2 && digits.bytes().all(|byte| byte.is_ascii_digit()))
            .ok_or_else(|| {
                Error::usage(format!(
                    "cannot derive a portable Linux wheel tag without a glibc floor in runtime target '{target}'; pass --wheel-tag"
                ))
            })?;
        let (major, minor) = floor.split_at(1);
        let architecture = if lower.contains("linux-x86_64") {
            "x86_64"
        } else if lower.contains("linux-aarch64") || lower.contains("linux-arm64") {
            "aarch64"
        } else {
            return Err(Error::usage(format!(
                "cannot derive a wheel architecture from runtime target '{target}'; pass --wheel-tag"
            )));
        };
        return Ok(format!("{python}-manylinux_{major}_{minor}_{architecture}"));
    } else if lower.contains("macos-") {
        return Err(Error::usage(format!(
            "cannot derive a macOS deployment floor from runtime target '{target}'; pass --wheel-tag"
        )));
    } else {
        return Err(Error::usage(format!(
...
mod consumer_entrypoint_tests {
    use super::{cmake_config_path_matches, derived_wheel_tag, terminal_safe_label};
    ...
    #[test]
    fn derived_linux_wheel_tags_preserve_the_locked_glibc_floor() {
        assert_eq!(
            derived_wheel_tag("linux-x86_64-glibc228-py313").unwrap(),
            "cp313-cp313-manylinux_2_28_x86_64"
        );
        assert_eq!(
            derived_wheel_tag("linux-aarch64-glibc231-py312").unwrap(),
            "cp312-cp312-manylinux_2_31_aarch64"
        );
        assert_eq!(
            derived_wheel_tag("linux-x86_64-glibc29-py311").unwrap(),
            "cp311-cp311-manylinux_2_9_x86_64"
        );
    }

    #[test]
    fn wheel_tags_require_an_explicit_unknown_deployment_floor() {
        for target in ["linux-x86_64-py313", "macos-aarch64-py313"] {
            let error = derived_wheel_tag(target).unwrap_err().to_string();
            assert!(error.contains("--wheel-tag"), "{target}: {error}");
        }
    }
}
